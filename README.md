...
> **히어로는 실제 이미지**(`assets/img/hero.webp`, `hero.png`)로 채워져 있습니다.
> 나머지 그래픽 자리는 점선 테두리의 **플레이스홀더 슬롯(`.art-slot`)** 으로 비워둔
> 상태입니다(행성 3개, 탐험가 초상, 교신 위성). 슬롯 안 내용을 실제 이미지/SVG로
> 교체하면 됩니다.
>
> 히어로는 **깊이맵 기반 WebGL 셰이더 시차**입니다. `hero.webp`(원본) + `hero_depth.png`(깊이맵)를
> 셰이더에서 합성해 스크롤에 따라 시차를 만듭니다. 하늘이 투명이라 뒤 별이 비칩니다.
>
> ⚠️ **현재 `hero_depth.png`는 임시(플레이스홀더) 깊이맵**입니다. 인물이 완전히 고정되는 최상 품질을
> 원하면 Depth Anything V2 등으로 만든 실제 깊이맵으로 교체하세요(제공된 `depth-parallax-lab.html`로 검증 가능).
> 셰이더 코드는 그대로 두고 파일만 바꾸면 됩니다.
>
> ⚠️ **셰이더는 http에서만 동작**합니다(`npm run dev`). 파일 더블클릭(`file://`)은 브라우저 CORS로
> 텍스처가 안 실려 **정적 폴백 이미지**만 보입니다.

## 여정 구조 (섹션)

1. **발사 (Hero)** — 퍼센트 카운터 프리로더(0→100%) → 히어로. 깊이맵 기반 셰이더 시차(스크롤 시 씬 안으로: 지면은 흐르고 인물은 고정). 투명 하늘 뒤로 별이 비침
