...
1. **발사 (Hero)** — 퍼센트 카운터 프리로더(0→100%) → 히어로. 3층 레이어 시차(스크롤 시 인물>로켓>지면 순으로 이동·확대). 투명 하늘 뒤로 별이 비침
...
- **히어로 WebGL 셰이더 배경** — "Mesh drift"(21st.dev) 셰이더를 검정·짙은 파랑
  딥스페이스 팔레트로 리컬러해 히어로 하늘에 은은한 유동 그라디언트를 깐 것.
  별(#space3d) 아래·이미지 뒤에 위치(z-index -2)하며, 히어로를 벗어나면
  불투명도 0으로 사라져 여명 전환과 충돌하지 않음. `prefers-reduced-motion`·
  WebGL 미지원 시 자동 비활성(정적 #bgLayer). 소스: `assets/js/shader-bg.js`
