...
- **낙하 별 배경(전 섹션 공통)** — 작은 노란 원(후광 포함)이 하늘에서 잔잔히
  내려오는 Canvas 2D 효과(`initStarfall`, `#space3d`). 고정 배경(z-index -1)으로
  콘텐츠 뒤에 깔려 **모든 섹션에서 동일하게** 보임(여명 구간 페이드아웃 없음).
  좌우 흔들림·크기·속도 랜덤, 화면 아래로 나가면 위에서 재생성.
  `prefers-reduced-motion` 시 정지 프레임, Canvas 미지원 시 CSS 별로 폴백
