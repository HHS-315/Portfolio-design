...
- **화이트 히어로 + 낙하 별** — 히어로 하늘은 화이트(#f8f8f8) 라인아트 무드.
  Canvas 2D로 4-point 스파클(별)을 이미지와 같은 얇은 검은 외곽선으로 그려
  잔잔히 떨어뜨림(좌우 흔들림·회전). 이미지(.frame) 뒤·투명 하늘에 표시되며
  히어로가 화면에 있을 때만 렌더. `prefers-reduced-motion` 시 정지 프레임.
  (`initFallingStars`, 히어로 위 nav는 자동으로 어두운 테마 전환)
