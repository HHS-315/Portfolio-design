...
> 교체하면 됩니다.
>
> 히어로 이미지는 **투명 배경(하늘 부분 투명)** 이라 그 뒤로 Three.js 별과
> 와이어프레임 행성이 비칩니다. 원본 17MB PNG는 투명도를 유지한 채
> WebP(123KB) + PNG(441KB, 폴백)로 최적화했습니다(폭 2560px).

## 여정 구조 (섹션)

1. **발사 (Hero)** — 퍼센트 카운터 프리로더(0→100%) → 히어로 이미지(로켓·인물, 전체 화면). 투명 하늘 뒤로 별·행성이 움직임
