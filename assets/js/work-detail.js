...
(function () {
  ...
  var WORK_DETAILS = {
    "yakbongji": {
      ...
      rows: [
        { h: "처방전 촬영", cap: "처방전을 직접 입력하거나 약 이름을 일일이 검색하는 과정 없이, 처방전을 촬영하는 것만으로 약의 정보를 확인할 수 있습니다. 촬영된 처방 정보를 바탕으로 복용약을 빠르게 확인하고 관리할 수 있도록 복약 경험의 시작 과정을 단순화했습니다.", vid: "assets/video/yakbongji_1.mp4" },
        { h: "복용약 비중 확인", cap: "등록된 복용약을 종류별로 분류하고 비중을 시각화해, 현재 어떤 종류의 약을 주로 복용하고 있는지 한눈에 파악할 수 있습니다. 개별 약의 정보를 확인하는 것을 넘어 전체적인 복용약의 구성을 보다 직관적으로 이해할 수 있도록 했습니다.", vid: "assets/video/yakbongji_2.mp4" },
        { h: "복용 일정 관리", cap: "등록한 약마다 복용해야 하는 시간을 설정하고, 설정한 시간에 맞춰 알림을 받아 복용 일정을 놓치지 않도록 관리할 수 있습니다. 약을 확인하는 과정에서 끝나지 않고 실제 복용 행동까지 자연스럽게 이어질 수 있도록 설계했습니다.", vid: "assets/video/yakbongji_3.mp4" }
      ],
    ...
    "stac": {
      ...
      role: "나의 역할은,\n이 문명을 시각화하는 것.",
      roleDesc: "STAC의 AI 기반 그래픽 디자인을 담당했습니다. 서비스의 핵심 컨셉인 ‘집중을 통해 성장하는 문명’을 시각적으로 전달하기 위해\n캐릭터 디자인부터 10단계의 문명 발전 배경, 단계별 캐릭터 변화, 아이콘까지 서비스 전반의 비주얼 에셋을 제작했습니다.",
      ...
      rows: [   // 이미지는 실제 파일 없어 플레이스홀더(교체 시 IMG["stac-rowN"] 또는 vid)
        { h: "캐릭터 디자인", cap: "Z세대가 친근하게 받아들일 수 있는 캐릭터를 중심으로 STAC의 캐릭터를 제작했습니다. 형태와 표정, 컬러를 기획자와 의견을 나누며 문명의 발전을 이끌어나가는 캐릭터를 AI로 제작했습니다.", vid: "assets/video/stac_sketch.mp4" },
        { h: "문명의 발전", cap: "사용자의 집중이 쌓일수록 세계가 발전한다는 서비스 컨셉을 시각적으로 전달하기 위해 총 10단계의 문명 발전 과정을 제작했습니다. 각 문명의 단계에 할당된 브릭이 목표치에 도달하면 배경이 전환되며, 각 문명의 특징이 건축물과 환경에 자연스럽게 드러나도록 시각적 변화를 구성했습니다.", vid: "assets/video/stac_list2.mp4" },
        { h: "움직이는 세계", cap: "영상 제작 AI로 캐릭터와 문명에 움직임을 더했습니다. 캐릭터의 행동과 환경의 변화를 자연스럽게 연결해, 사용자가 자신의 집중으로 실제로 성장하고 있다는 감각을 느낄 수 있도록 모션 그래픽으로 확장했습니다.", vid: "assets/video/stac_list1.mp4" },
        { h: "UI 그래픽 제작", cap: "서비스 내 다양한 기능과 정보를 시각적으로 전달하기 위해 AI를 활용해 UI 아이콘과 그래픽 에셋을 제작했습니다. 캐릭터와 문명에서 제작한 형태와 질감, 분위기를 아이콘에도 일관되게 반영해, 비주얼 컨셉이 자연스럽게 이어지도록 구성했습니다.", vid: "assets/video/stac_list3.mp4" }
      ],
    ...
    "company-renewal": {
      ...
      link: "http://aitom.biz/main/main.php",    // "이번 프로젝트는." 통계 섹션 바로 위에 노출할 실제 사이트 링크(있는 키만 렌더)
      paras: [],
      caps: ["IA / Structure", "Visual System", "Responsive", "Handoff"],
      rows: [
        { h: "핵심 정보의 재구성", cap: "기존 웹사이트의 여러 정보를 사용자가 빠르게 이해할 수 있도록 핵심 사업과 기술 정보를 중심으로 콘텐츠의 우선순위를 재정리했습니다. 주요 메시지와 시각 자료를 함께 배치해 긴 설명을 읽지 않아도 회사가 가진 기술과 서비스의 특징을 직관적으로 파악할 수 있도록 구성했습니다." },
        { h: "정보 탐색 흐름의 설계", cap: "기술과 서비스 소개에서 성과와 글로벌 현황으로 자연스럽게 이어지도록 사용자가 회사를 이해하는 순서에 맞춰 정보 흐름을 설계했습니다. 각각의 콘텐츠를 독립적으로 나열하기보다 스크롤을 따라 회사의 역량과 성장 과정을 단계적으로 이해할 수 있도록 구성했습니다." },
        { h: "정보와 이미지의 균형", cap: "기술과 성과를 전달한 뒤 기업의 가치와 업무 환경을 이미지 중심으로 보여주며, 긴 페이지에서도 사용자가 흐름을 놓치지 않도록 하는것을 고려했습니다. 텍스트와 이미지의 비중을 적절히 조절해 시각적 피로를 줄이고, 기업의 기술력뿐만 아니라 조직이 추구하는 가치와 문화까지 자연스럽게 이해할 수 있도록 구성했습니다." }
      ],
      stats: {   // TODO: Tools·Duration 실제 값으로 교체
        h: "이번 프로젝트는.",
        items: [
    ...
    "maritime": {
      en: "Maritime Institute", title: "해양수산정책기술연구소 사이트", sub: "연구기관의 전문성과 신뢰를\n명확한 정보와 시각적 경험으로 전달하는 웹사이트",
      lead: "해양수산 분야의 전문성과 연구기관의 신뢰도를 직관적으로 전달할 수 있도록 해양수산정책기술연구소의 웹사이트를 디자인했습니다.\n기관의 정보 전달 구조를 명확하게 정리하는 동시에, 이미지와 그래픽 요소를 적극적으로 활용해 일반적인 연구기관 웹사이트의 정적인 인상에서 벗어난 시각적 경험을 구축했습니다.\n\n기관의 신뢰성과 전문성을 유지하면서, 정보 중심의 연구기관 웹사이트에 보다 선명한 시각적 정체성을 더했습니다.\n콘텐츠의 성격에 따라 다양한 레이아웃을 적용하되 공통된 디자인 언어를 유지해, 연구소가 가진 정보와 방향성을 일관된 경험으로 전달할 수 있도록 완성했습니다.",   // 마무리 문단(기존 outro)을 리드 아래에 단락(\n\n=빈 줄) 띄워 배치
      leadSm: true,   // 다른 서브페이지 소개 문구와 동일 크기·굵기(14px/weight 400)
      shotVid: "assets/video/marine_hero.mp4",   // 큰 단일 이미지(.wd__shot)를 영상으로 (company_web과 동일 방식)
      paras: [],
      caps: ["IA / Structure", "Accessibility", "Visual System", "Responsive"],
      rows: [
        { h: "전문성을 시각적 경험으로", cap: "연구기관이 가진 전문성과 신뢰성을 유지하면서도 딱딱하고 정적인 인상을 줄이기 위해 정보와 비주얼이 함께 어우러지는 화면 구성을 설계했습니다. 연구소의 주요 콘텐츠를 명확한 위계로 정리하고, 과감한 이미지 배치와 여백을 활용해 각 섹션이 자연스럽게 이어지는 흐름을 만들었습니다.", vid: "assets/video/marine_list1.mp4" },
        { h: "복잡한 정보를 명확하게", cap: "연혁과 조직도처럼 정보량이 많은 콘텐츠는 단순한 텍스트 나열에서 벗어나 정보 간의 관계와 구조가 한눈에 파악되도록 시각화했습니다. 연혁은 시간의 흐름을 따라 탐색할 수 있는 타임라인으로, 조직도는 각 조직의 관계를 중심으로 재구성해 정보 탐색의 부담을 줄였습니다.", vid: "assets/video/marine_list2.mp4" },
        { h: "하나의 흐름으로 연결된 화면", cap: "각 페이지마다 서로 다른 성격의 정보를 다루면서도 블루 컬러, 곡선형 그래픽, 타이포그래피와 여백 규칙을 일관되게 적용했습니다. 이를 통해 메인 비주얼부터 기관 소개, 연혁, 조직도, CI, 오시는 길까지 흐름이 자연스럽게 연결되도록 구성했습니다.", vid: "assets/video/marine_list3.mp4" }
      ],
      stats: {
        h: "이번 프로젝트는.",
        items: [
  ...
  var IMG = {
    ...
    // 해양수산 — 텍스트+이미지 행(3)은 marine_list1~3.mp4 영상(rows[].vid). 아래 jpg는 그 영상의 poster(디코드 전 폴백)
  ...
  function riseInner(text) { return '<span class="wd-rise__i">' + esc(text).replace(/\n/g, "<br>") + "</span>"; }   // \n → line break (e.g. a two-line lead)
  ...
  function populate(key) {
    ...
    elRows.innerHTML = (d.rows || []).map(function (r, i) {
      ...
      return '<div class="wd__row">' +
          '<div class="wd__row-text">' +
            (r.kicker ? '<p class="wd__row-kicker wd-rise">' + riseInner(r.kicker) + "</p>" : "") +
    ...
    // 실제 사이트 링크 — "이번 프로젝트는." 통계 섹션 바로 위. d.link 있는 키만 렌더(없으면 :empty → 마진 collapse).
    ...
    elLink.innerHTML = !d.link ? "" :
      '<span class="wd-rise__i"><a class="wd__link-a" href="' + esc(d.link) + '" target="_blank" rel="noopener noreferrer">' +
        '<span class="wd__link-url">사이트 바로가기</span>' +
