...
(function () {
  ...
  var WORK_DETAILS = {
    ...
    "maritime": {
      ...
      rows: [
        { h: "전문성을 시각적 경험으로", cap: "연구기관이 가진 전문성과 신뢰성을 유지하면서도 딱딱하고 정적인 인상을 줄이기 위해 정보와 비주얼이 함께 어우러지는 화면 구성을 설계했습니다. 연구소의 주요 콘텐츠를 명확한 위계로 정리하고, 과감한 이미지 배치와 여백을 활용해 각 섹션이 자연스럽게 이어지는 흐름을 만들었습니다." },
        { h: "복잡한 정보를 명확하게", cap: "연혁과 조직도처럼 정보량이 많은 콘텐츠는 단순한 텍스트 나열에서 벗어나 정보 간의 관계와 구조가 한눈에 파악되도록 시각화했습니다. 연혁은 시간의 흐름을 따라 탐색할 수 있는 타임라인으로, 조직도는 각 조직의 관계를 중심으로 재구성해 정보 탐색의 부담을 줄였습니다." },
        { h: "하나의 흐름으로 연결된 화면", cap: "각 페이지마다 서로 다른 성격의 정보를 다루면서도 블루 컬러, 곡선형 그래픽, 타이포그래피와 여백 규칙을 일관되게 적용했습니다. 이를 통해 메인 비주얼부터 기관 소개, 연혁, 조직도, CI, 오시는 길까지 흐름이 자연스럽게 연결되도록 구성했습니다." }
      ],
      stats: {
        h: "이번 프로젝트는.",
        items: [
