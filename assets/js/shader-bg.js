/* ============================================================================
   ...
   JS. One fixed WebGL canvas behind every section, tinted to a deep-space
   palette (black + dark blue) so it reads as the night sky. Runs page-wide
   (all sections); the falling yellow stars (#space3d) sit above it and the
   content composites on top. Pauses when the tab is hidden. No dependencies.
...
void main(){
  ...
  const UNIFORMS = {
    colors: [
      [0.012, 0.024, 0.055], // near-void black-blue (~#060E)
      [0.035, 0.063, 0.140], // deep navy
      [0.075, 0.130, 0.280], // muted dark blue
      [0.130, 0.185, 0.360], // faint indigo glow
      [0.130, 0.185, 0.360],
      [0.130, 0.185, 0.360],
      [0.130, 0.185, 0.360],
      [0.130, 0.185, 0.360],
  ...
  function initShaderBackground() {
    ...
    if (!gl) return; // no WebGL → falls back to the dark #bgLayer colour
    ...
    if (reduce) { render(0); return; } // static frame, no animation

    let raf = 0, running = false;
    const start0 = performance.now();
    const loop = now => { resize(); render(((now - start0) / 1000) * UNIFORMS.timeScale); raf = requestAnimationFrame(loop); };
